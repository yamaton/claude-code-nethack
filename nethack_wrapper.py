...
class NetHackWrapper:
    ...
    def start_game(self, character_name: str = "Claude") -> bool:
        """Start a new NetHack game with specified character."""
        try:
            # Start nethack with options to skip some prompts
            cmd = f"nethack -u {character_name}"
            self.process = pexpect.spawn(cmd, timeout=15)
            ...
            if index == 0:  # Restoring save file
                ...
                self.process.expect("--More--", timeout=15)
